	    	...
	    	if(fonte == adiarConfirmarBtn) {
	    		...
	    		try {
		    		if(!pastaBackupFld.getText().isEmpty() && (proximaHoraBox.isVisible() && proximaHoraBox.getValue() != null)) {
						...
						Alerta.getInstance().imprimirMsg("Alerta", "É necessário selecionar pasta e informar hora do backup de amanhã antes da realização de backup", AlertType.WARNING);
