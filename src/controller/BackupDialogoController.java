	    	...
	    	if(fonte == adiarConfirmarBtn) {
	    		...
	    		try {
		    		if(!pastaBackupFld.getText().isEmpty() && (proximaHoraBox.isVisible() && proximaHoraBox.getValue() != null)) {
						...
						Alerta.getInstance().imprimirMsg("Alerta", " É necessário selecioanr a pata e informar a hora de backup de amanhã antes de realização de backup", AlertType.WARNING);
