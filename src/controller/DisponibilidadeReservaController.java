    	...
    	try {
	    	...
	    	if(fonte == atualizarTabelaBtn) {
	    		if(toggleGroup.getSelectedToggle()!= null) {
		    		...
		    		Alerta.getInstance().imprimirMsg("Busca Concluída", disponiTbl.getItems().size()+" resultados",AlertType.INFORMATION);
	    		}else
	    			Alerta.getInstance().imprimirMsg("Alerta", "É necessário selecionar uma opção de busca",AlertType.WARNING);
	    		...
	    		if(funcionario.getFilial() != null) {
	    			...
	    			Alerta.getInstance().imprimirMsg("Alerta", "O funcionário seleceionado não está associado a nenhuma filial",AlertType.WARNING);
	    		}
	    	}
    	}catch (BoException e) {
