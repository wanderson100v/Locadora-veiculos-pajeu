    	...
    	try {
    		...
    		else if(fonte == selectMotoristaBtn) 
    		{
	    		if(entregaDate.getValue() != null) {
	    			...
	    			Alerta.getInstance().imprimirMsg("Alerta","É necessário selecionar a data de retirada para só em seguida selecionar o motorista", AlertType.WARNING);
    ...
    void checkHandle(ActionEvent event) {
    	...
    	else if(fonte == outraFilialCk)
    	{
    		if(funcionario.getFilial() != null) {
    			...
    			Alerta.getInstance().imprimirMsg("Alerta", "Filial do funcionário selecionado não pode ser utilizada. O Funcionário n�o está associado a nenhuma filial",AlertType.WARNING);
