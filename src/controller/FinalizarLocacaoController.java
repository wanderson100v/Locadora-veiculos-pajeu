		...
		try {
			...
			if (e.getSource() == devuAtualCk)
				...
				if (etCk.isSelected()
					...
					errosEmTela.append(
							"- Abastecimento para veículo selecionado, desta forma, É necessário definir data e hora para a execução do abastecimento\n");
				if (lpCk.isSelected()
						&& (!lpAtualCk.isSelected() && (lpDate.getValue() == null || lpHoraBox.getValue() == null)))
					errosEmTela.append(
							"- Limpeza para veículo selecionada, É necessário tamb�m, definir data e hora para a execução da limpeza\n");
				if (!revisaoPane.isDisable() && (rsDate.getValue() == null || rsHoraBox.getValue() == null
						|| rsCustoFld.getText().trim().isEmpty()))
					errosEmTela.append("- A nova quilometragem do veículo atualizada requer o agendamento de uma revisão\n");
