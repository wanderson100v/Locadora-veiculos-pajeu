		...
		try {
			...
			if(locacao.getReservaOrigem()!= null) {
				...
				notaFiscal.append(  
						...
						+ "\nser�o definidos pela categoria do veículo locado:"
						+ "\n\t- Valor por Diaria = "+valorDiaria);
				if(locacao.getTipoLocacao() == TipoLocacao.KM_LIVRE) {
			...
			if(horasAposTerminoPrevistoDiaria >0)
				if(horasAposTerminoPrevistoDiaria <=4) {
					...
					notaFiscal.append("\n\t- Valor horas restantes � mais uma diaria = "+valorDiaria);
			...
			if(limpeza || abastecer) {
				notaFiscal.append("\n- Taxas referêntes a manutenão de veículo");
				if(limpeza) {
			...
			throw new BoException("É necessário o tipo, hora de retirada/decolução da locação"
